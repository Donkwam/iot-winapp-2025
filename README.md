...
- 윈앱 응용개발
    ...
    <img src="./image/cs0014.png" width="700">
...
- 데이터바인딩 - [디자인](./day07/Day07Study/WpfStudyApp04/MainWindow.xaml),  - [소스](./day07/Day07Study/WpfStudyApp04/MainWindow.xaml.cs)
    ...
    - DataContext : Xaml이 데이터를 어디에서 가져오는지 설정하는 속성
        ...
        - 클래스 객체를 패키지로 전달 가능
    - 바인딩 순서
        1. WPF 바인딩 엔진은 DataContext 객체 안에서 public접근제어자의 fruitItem라는 값을 검색
...
- 데이터베이스 데이터 바인딩
    ...
    3. 비하인드 코드에서 MySQL 소스 추가
        ...
        - 명렬실행객체 : MySqlCommand(쿼리문 실행)
        - 실행결과리더객체 : MySqlDataReader(쿼리결과 데이터)
        - 트렌잭션객체 : 옵션. 트랜잭션 처리시 필요
...
- 컨트롤 디자인, 리소스 - [소스](./day08/Day08Study/WpfStudyApp04/MainWindow.xaml)
    - WPF는 컨트롤에 디자인도 맘데로 변경가능
    - 리소스 : 컨트롤의 공유화
        - APP.xaml Application.Resources 에 필요한 컨트롤 디자인을 정의
        - 각각의 Window, Page.xaml에 선언한 리소스 > StaticResource
...
- 기본 사용법 - [소스](./day09/Day09Study/WpfStudyApp07/MainWindow.xaml)
    ...
    - App.xaml.cs 오픈 - [소스](./day09/Day09Study/WpfStudyApp07/App.xaml)
        ```cs
    ...
    - MainWindow.xaml xmlns 추가. App.xaml과 동일
        ```xml
...
Windows Forms(C#) 기반의 간단한 가계부 프로그램입니다.  
사용자는 수입/지출 내역을 입력하고, 저장하거나 불러와 관리할 수 있습니다.

---

#### 주요 기능

- 날짜, 항목, 금액, 수입/지출 구분 입력 및 추가
- 내역 목록 출력 (DataGridView)
- 총 수입 / 총 지출 / 총 잔액 실시간 계산
- `.txt` 파일로 저장 및 불러오기 지원
- 종료 시 확인 메시지 표시
...
1. **항목 입력**
   - 날짜 선택 → 항목명 입력 → 금액 입력 → 수입/지출 선택 → `추가` 버튼 클릭

2. **파일 저장**
   - 메뉴 → `파일 > 저장` 선택 → 원하는 경로에 `.txt` 파일 저장  
   - 저장 형식 예시:  
     ```
     2025-05-12|급여|3000000|수입
     2025-05-12|커피|4500|지출
     ```

3. **파일 불러오기**
   - 메뉴 → `파일 > 불러오기` 선택 → `.txt` 파일 선택 후 불러오기 진행
